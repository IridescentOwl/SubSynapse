...
export class SubscriptionGroupController {
  ...
  public static async storeCredentials(req: AuthenticatedRequest, res: Response): Promise<Response> {
    ...
    try {
      ...
      let encryptedCredentials = cipher.update(credentials, 'utf8', 'hex');
      ...
      return res.status(201).json({ message: 'Credentials stored successfully' });
    } catch (error) {
  ...
  public static async getCredentials(req: AuthenticatedRequest, res: Response): Promise<Response> {
    ...
    try {
      ...
      let decryptedCredentials = decipher.update(credentials.encryptedCredentials, 'hex', 'utf8');
