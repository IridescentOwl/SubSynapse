...
// Ensure new users have admin-required fields
export const register = async (
  ...
  const newUser: User = {
    ...
    // admin-related fields expected by the portal
    role: "user",
    isActive: true,
    createdAt: nowIso,
  };
...
export const fetchMySubscriptions = async (): Promise<MySubscription[]> => {
  ...
  const myMembershipIds = memberships
  ...
  return myGroupData.map((group) => {
    ...
    return {
      ...
      credentials: {
        // Only send credentials to members
...
export const leaveGroup = async (
  ...
  memberships = memberships.filter(
    (m) => !(m.userId === user.id && m.groupId === subscriptionId)
  );
...
export type Withdrawal = (typeof MOCK_WITHDRAWAL_REQUESTS)[number];
...
// Update a user (role, isActive, name, etc.)
...
export const adminDeleteUser = async (id: string): Promise<void> => {
  ...
  memberships = memberships.filter((m) => m.userId !== id);
};

// Set group status (approve, mark full, etc.)
export const adminSetGroupStatus = async (
  groupId: string,
...
export const adminListWithdrawals = async (): Promise<Withdrawal[]> => {
  await delay(400);
  return withdrawalRequests;
};

// Approve or reject a withdrawal
export const adminSetWithdrawalStatus = async (
  id: string,
  status: "approved" | "rejected"
): Promise<void> => {
  await delay(300);
  const w = withdrawalRequests.find((w) => w.id === id);
